...
const ELECTROBUN_ROOT = resolve(dirname(require.resolve("electrobun")), "..", "..", "..");

function getTestZstdBinary() {
  const platformDir = (() => {
    if (process.platform === "darwin") {
      return `dist-macos-${process.arch}`;
    }
    if (process.platform === "linux") {
      return `dist-linux-${process.arch}`;
    }
    if (process.platform === "win32") {
      return `dist-win-${process.arch}`;
    }
    return null;
  })();
  const binaryName = process.platform === "win32" ? "zig-zstd.exe" : "zig-zstd";
  if (platformDir) {
    const bundledBinary = join(ELECTROBUN_ROOT, platformDir, binaryName);
    if (existsSync(bundledBinary)) {
      return bundledBinary;
    }
  }

  const fallback = Bun.which(binaryName) || Bun.which("zstd");
  if (fallback) {
    return fallback;
  }

  throw new Error(`Unable to locate ${binaryName} or zstd for Bunny Ears integration tests.`);
}
...
process.env.BUNNY_EARS_ZSTD_BIN = getTestZstdBinary();

const { buildCarrotSource } = await import("../src/bun/carrotBuilder");
const GoldfishDB = (await import("goldfishdb")).default;
